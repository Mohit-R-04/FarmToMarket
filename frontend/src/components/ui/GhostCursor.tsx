...
const GhostCursor: React.FC<GhostCursorProps> = ({
  ...
  `;
    ...
    // Spark/Firework function
    vec4 firework(vec2 p, vec2 mousePos, float intensity, float activity) {
      ...
      // Sparks generation
      // Use time and position to create chaotic sparkling
      float noiseVal = hash(p * 80.0 + iTime * 20.0); 
      
      // Radial burst feel
      vec2 dir = normalize(p - mousePos);
      float angle = atan(dir.y, dir.x);
      float radialNoise = hash(vec2(angle * 10.0, iTime * 5.0));
      
      // Combine noises for "cracker" look
      float sparks = smoothstep(0.95, 1.0, noiseVal * radialNoise);
      
      // Fade sparks with distance but keep them sharp
      sparks *= exp(-d * 3.0);
      
      // Combined alpha
      float alpha = (glow + sparks * 3.0) * activity;
      
      // Color mixing: White hot center -> Base Color -> Darker edges
      vec3 hotColor = vec3(1.0, 1.0, 0.8); // White-yellow
      vec3 color = mix(iBaseColor, hotColor, sparks + glow * 0.5);
    ...
    void main() {
      ...
      // Current mouse position burst
      vec4 b = firework(uv, mouse, 1.0, iOpacity);
      colorAcc += b.rgb;
      alphaAcc += b.a;
      ...
      for (int i = 0; i < MAX_TRAIL_LENGTH; i++) {
        ...
        t = pow(t, 0.5); // Slower fade for longer trail
        if (t > 0.01) {
          // Jitter the trail positions slightly for more chaotic "spark" feel
          float jitter = hash(vec2(float(i), iTime)) * 0.02;
          vec4 bt = firework(uv, pm + vec2(jitter), t * 0.6, iOpacity);
          colorAcc += bt.rgb;
          alphaAcc += bt.a;
        }
      }
